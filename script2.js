...
function getTossteamName(toosWin, tossLose) {
  document.getElementById("winnerTeam").innerHTML =
    toosWin + " is won the Toss.....Create your team.";
  currentTeam = toosWin;
  tossWinTeam = toosWin;
...
function showPlayers(playerArray, mainArray) {
  for (let temp of mainArray) {
    ...
    document.getElementById("showPlayer").innerHTML +=
      ...
      temp.credit +
      sp +
      `<button id= "${temp.name}" onclick="addPlayerHandler('${temp.name}',${
        temp.credit
      })"${playerInclude ? " disabled" : " "}>Add</button>` +
      sp +
      `<button id= "${temp.name}${
        temp.credit
      }"  onclick="removePlayerHandler('${temp.name}')" ${
...
function displaySummery(array) {
  for (let temp of array) {
    document.getElementById("showPlayer").innerHTML +=
      temp.name + br + temp.playingRole + br + temp.credit + br;
    let opt = temp.name;
...
function addPlayers(teamArray) {
  ...
  document
    .getElementById(`${member.name}${member.credit}`)
...
function next() {
  if (currentTeam == tossWinTeam) {
    ...
    if (checkValidation) {
      creditScore = 0;
      document.getElementById("winnerTeam").innerHTML =
        "Now " + tossLoseTeam + " Create your team.";
      document.getElementById("credit").innerHTML = "Credit : " + creditScore;
...
function addPointToTeam(captain, viceCap, teamObj) {
  typeof shotType == "number"
    ? (teamObj.teamRun = teamObj.teamRun + shotType)
    : "";
  currentBatsman == captain
    ? (batMul = 2)
    : currentBatsman == viceCap
    ? (batMul = 1.5)
    : (batMul = 1);
  currentBowler == captain
    ? (bowlMul = 2)
    : currentBowler == viceCap
    ? (bowlMul = 1.5)
    : (bowlMul = 1);
  if (shotType == 4) {
...
function scorebord() {
  if (tossWinTeamPoint.teamPoints > tossLoseTeamPoint.teamPoints) {
    document.getElementById("winerteam").innerHTML =
      tossWinTeam + " is won the match";
  } else {
    document.getElementById("winerteam").innerHTML =
      tossLoseTeam + " is won the match";
  }
  document.getElementById("1st inning").innerHTML =
    `1st inning - ${tossWinTeam}` + `Points:(${tossWinTeamPoint.teamPoints})`;
  document.getElementById("2nd inning").innerHTML =
    `2nd inning - ${tossLoseTeam}` + `Points:(${tossLoseTeamPoint.teamPoints})`;
...
function playMatch() {
  ...
  document.getElementById("vice-captain").innerHTML =
    "Vice-Captian : " + team1ViceCap;
  ...
  document.getElementById("bowlingTeamName").innerHTML =
    tossLoseTeam + "(BOWLING)";
  document.getElementById("captain2").innerHTML = "Captain : " + team2Cap;
  document.getElementById("vice-captain2").innerHTML =
    "Vice-Captian : " + team2ViceCap;
...
function updateScoreboard(team1Obj, team2Obj) {
  ...
  document.getElementById("battingTeamPoint").innerHTML =
    "Points : " + team1Obj.teamPoints;
  document.getElementById("teamRuns").innerHTML = "Runs: " + team1Obj.teamRun;
  document.getElementById("Wicket").innerHTML =
    "Wicket : " + team1Obj.teamWicket;
  document.getElementById("bowlingTeamPoint").innerHTML =
    "Points : " + team2Obj.teamPoints;
  document.getElementById("over").innerHTML = "Overs :" + ballCount;
  document.getElementById("information").innerHTML +=
    "Ball " + ball + " - " + currentDate + " - " + shotType + br;
}
function playSecInning() {
  currentTeam = tossLoseTeam;
  ballCount = 0;
  document.getElementById("battingTeamName").innerHTML =
    tossLoseTeam + "(BATTING)";
  document.getElementById("battingTeamPoint").innerHTML =
    "Points :" + tossLoseTeamPoint.teamPoints;
  document.getElementById("teamRuns").innerHTML =
    "Runs :" + tossLoseTeamPoint.teamRun;
  document.getElementById("Wicket").innerHTML =
    "Wicket :" + tossLoseTeamPoint.teamWicket;
  ...
  document.getElementById("over").innerHTML =
    "Overs :" + tossWinTeamPoint.teamOver;
  document.getElementById("bowlingTeamPoint").innerHTML =
    "Points :" + tossWinTeamPoint.teamPoints;
